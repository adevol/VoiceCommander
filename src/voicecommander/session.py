...
class RecordingSession:
    ...
    def _run_preview(self) -> None:
        try:
            ...
            for update in transcribe_live(
                self._recorder, self.model.result(), self.settings, self._stop, self._languages
            ):
    ...
    def wait_preview(self) -> tuple[Settings, PreviewText | None]:
        """Return a fixed snapshot only after the worker can no longer change it."""
        with self._drain_lock:
            return self._drain_preview()

    def _drain_preview(self) -> tuple[Settings, PreviewText | None]:
        if self._worker is not None:
            self._worker.join(timeout=0.5)
            if self._worker.is_alive():
                if self.model is not None and self.model.done() and not self.model.cancelled():
                    if self.model.exception() is None:
                        self.model.result().cancel_preview()
                self._worker.join()
            self._worker = None
